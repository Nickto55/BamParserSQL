...
зависимостей
const
deps = await eel.check_dependencies()();
if (!deps.success) {
...
БД
const
dbTest = await eel.test_db_connection()();
if (dbTest.success) {
...
{
    const
result = await eel.select_files('отчетов')();
if (result.success)
{
...
{
    const
result = await eel.open_result_file()();
if (!result.success)
{
...
const
result = await eel.open_work_table()();
if (result.success) {
...
{
    const
result = await eel.get_help_text()();
...
{
    const
container = document.getElementById('status-text');
const
line = document.createElement('div');
line.className = 'log-line';
if (color)
line.classList.add(`log -${color}
`);
...
function
processingFinished(path, wasStopped)
{
    isProcessing = false;
